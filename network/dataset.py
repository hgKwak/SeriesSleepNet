...
#     def __init__(self, x, y, data_aug, seq_len):
#         x = torch.FloatTensor(np.transpose(x, [0, 2, 1]))
#         fs = x.shape[-1] // 30
#         sliding_step = int(fs * 0.1)
#         y = torch.LongTensor(y)
#         self.y_data = y
#         self.len = len(y)
#         x_tmp = []
...
#                 y_tmp.append(y_)
#
#             self.x_data = torch.nn.utils.rnn.pad_sequence(x_tmp, batch_first=True)
#             self.y_data = torch.nn.utils.rnn.pad_sequence(y_tmp, batch_first=True)
#
#         else:
#             for i in range(0, len(x), seq_len):
#                 x_ = x[i: i + seq_len]
#                 x_tmp.append(x_)
#             self.x_data = torch.cat(x_tmp)
...
class LSTMDataset(Dataset):
    def __init__(self, x, y, seq_len, data_aug, n_slidings=4, step_sec=1):
...
def load_dataset(data_dir, input_type, cv_idx):
    ...
    if input_type == 'SHHS' or input_type == 'male_SHHS' or input_type == 'female_SHHS' \
            or input_type == 'old_SHHS' or input_type == 'young_SHHS':
